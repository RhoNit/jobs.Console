...
from db.repository.jobs import create_new_job, get_job_by_id, get_all_jobs, search_job
...
@router.get("/")
async def home(request: Request, db: _orm.Session = Depends(get_db), msg: str = None):
...
def job_detail(id: int, request: Request, db: _orm.Session = Depends(get_db)):
...
def create_job(request: Request, db: _orm.Session = Depends(get_db)):
    return templates.TemplateResponse("jobs/create_job.html", {"request": request})


@router.post("/post-a-job/")
async def create_job(request: Request, db: _orm.Session = Depends(get_db)):
    ...
    if form.is_valid():
        try:
            ...
            print(e)
            form.__dict__.get("errors").append("You might not be logged in! in case problem persists please raise a PR")
...
def show_jobs_to_delete(request: Request, db: _orm.Session = Depends(get_db)):
    jobs = get_all_jobs(db=db)
    return templates.TemplateResponse("jobs/show_jobs_to_delete.html", {"request": request, "jobs": jobs})


@router.get("/search")
def search(request: Request, db: _orm.Session = Depends(get_db), query: Optional[str] = None):
